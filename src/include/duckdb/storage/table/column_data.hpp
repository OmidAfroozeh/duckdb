...
public:
	...
	idx_t Scan(TransactionData transaction, idx_t vector_index, ColumnScanState &state, Vector &result, optional_ptr<ClientContext> context = nullptr);
	...
	virtual void Filter(TransactionData transaction, idx_t vector_index, ColumnScanState &state, Vector &result,
	                    SelectionVector &sel, idx_t &count, const TableFilter &filter, TableFilterState &filter_state, optional_ptr<ClientContext> context);
