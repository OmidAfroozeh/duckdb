...
unique_ptr<LogicalOperator> USSR_optimizer::Get_USSR_Expressions(unique_ptr<LogicalOperator> op,
	...
	for (auto &expression : old_expressions) {
		if (expression->return_type == LogicalType::VARCHAR) {
			auto &child_colref_expr = expression->Cast<BoundColumnRefExpression>();
			auto colref_expr = make_uniq<BoundColumnRefExpression>(LogicalType::VARCHAR, child_colref_expr.binding);
