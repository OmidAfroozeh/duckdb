...
OperatorResultType PhysicalUnifiedString::Execute(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
		...
		if (size.GetIndex() <= 500) {
			...
			if (insert_to_ussr[col_idx] &&
				...
				for (idx_t i = 1; i < state.count.size(); i++) {
					if (state.count[i] > (2 * state.current_analysis_count)) {
				...
				vector<idx_t> priority_selection;
				for (idx_t i = 1; i < state.count.size(); ++i) {
					if (state.count[i] > (10) && !state.inserted[i]) {
						priority_selection.push_back(i);
						state.inserted[i] = true;
					}
				}
	...
	//
	//		if(insert_to_ussr[col_idx] && DictionaryVector::DictionaryId(input.data[col_idx]) !=
	//state.current_dict_ids[col_idx]){
	//
	//
	//
	//
