	...
	for (idx_t col_idx = 0; col_idx < input.data.size(); ++col_idx) {
		...
		if ((size.GetIndex() <= 0 && dict_encoded_val_size.GetIndex() > 10000) || (dict_encoded_val_size.GetIndex() / size.GetIndex()) > 10000000) {
			...
			if (insert_to_ussr[col_idx] &&
				...
				if (size.GetIndex() > state.inserted[col_idx].size()) {
					state.inserted[col_idx].resize(size.GetIndex(), false);   // one memset
					state.count[col_idx].resize(size.GetIndex(), 0);          // one memset
				} else {
					std::fill_n(state.inserted[col_idx].begin(), size.GetIndex(), false);
					std::fill_n(state.count[col_idx].begin(),    size.GetIndex(), 0);
				}
